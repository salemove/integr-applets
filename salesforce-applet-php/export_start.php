...
if($json)
{
    ...
    $sfExists = checkSF($config["sfObject"],"Email",$array["visitorEmail"]);
	
	if(!$sfExists["records"][0]["Id"])
	{
        // Create SF Lead record //
        $name = explode(" ",$array["visitorName"]);
		$data = array(
            "firstName" => $name[0],
            "lastName" => $name[1],
			"Email" => $array["visitorEmail"],
            "Phone" => $array["visitorPhone"],
            //"Company" => "Glia Chat"
		);
        $createdInfo = json_decode(createSF($config["sfObject"],json_encode($data)),true);
        writeToFile("CreatedInfo",$createdInfo,"a+");
        if($createdInfo["id"]) {
            writeToFile("CreatedInfo Start",$createdInfo["id"],'a+');
            updateCA($array["visitorId"],array("custom_attributes" => array("externalId" => $createdInfo["id"])));
        }
		exit;
	} else {
		updateCA($array["visitorId"],array("custom_attributes" => array("externalId" => $sfExists["records"][0]["Id"])));
		exit;
	}
...
function updateCA($visitorId,$cData)
{
	include("config.php");
	if($_SESSION["bearerToken"])
	{
    ...
    $cData["custom_attributes_update_method"] = "replace";

    $input = json_encode($cData);
	$url = "https://api.salemove.com/sites/".$config["gliaSiteId"]."/visitors/".$visitorId;
    ...
    writeToFile("Update Input",$input,"a+");
    $return = curl_exec($ch);
    writeToFile("Update CA --",$return,"a+");
    curl_close($ch);	
	
	//curlPatch($url,json_encode($cData),$cHeaders,true);
	return true;
}
